...
# Remove CCM uninitialized taint from Windows nodes
# BYOH nodes are manually provisioned and don't need CCM initialization
# This taint prevents pods without the toleration from scheduling
echo "=== Removing CCM taint from Windows nodes ==="
for node in $(oc get nodes -l kubernetes.io/os=windows -o jsonpath='{.items[*].metadata.name}'); do
  echo "Removing CCM taint from node: ${node}"
  oc adm taint nodes "${node}" node.cloudprovider.kubernetes.io/uninitialized- 2>/dev/null || echo "  (taint not present or already removed)"
done

# Display Windows node taints after cleanup
echo "=== Windows node taints (after cleanup) ==="
