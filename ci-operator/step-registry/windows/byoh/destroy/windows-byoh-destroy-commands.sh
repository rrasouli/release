...
# WORKAROUND: For UPI, override platform to "none" (same as provision step)
# UPI AWS clusters report platform=aws in Infrastructure CR but use platform=none for Terraform
# AWS_REGION file exists only in UPI workflows (created by aws-windows-ami-discover step)
if [[ -f "${SHARED_DIR}/AWS_REGION" ]]; then
    echo "Detected UPI workflow (AWS_REGION file exists)"
    echo "Overriding platform from '${PLATFORM}' to 'none' for UPI cleanup"
    PLATFORM="none"
fi
